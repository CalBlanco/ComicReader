...
class ReaderScreen(tk.Frame):
    def __init__(self,master):
        ...
        self.comic_reader = Reader('./best.pt') #init reader 
        self.voicer = Voicer("./voices/omniman/omniman.wav")
        ...
        read_mode_button = ttk.Button(mode_frame, text="Read Mode", command= lambda: self.comic_reader.read_process('read'))
        save_mode_button = ttk.Button(mode_frame, text="Save Mode(copy pictures)", command= lambda: self.comic_reader.read_process('save'))

        read_mode_button.grid(row=0,column=0, padx=10)
        save_mode_button.grid(row=0,column=2, padx=10)
        stop_button = ttk.Button(mode_frame,text="Stop", command=lambda: self.comic_reader.proc_stop())
        ...
        do_meta = ttk.Button(meta_tab, text=f'Read Meta', command=lambda: self.pickMeta())
        do_meta.pack()
        meta_tab.pack()
    ...
    def pickMeta(self):
        path = path = filedialog.askopenfilename(filetypes=(('JSON', 'meta.json'),("All files", "*.*")))
        if path:
            self.voicer.readFromMeta(path,True)
